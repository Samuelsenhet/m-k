...
**Where:** `src/lib/matching.ts`, Supabase Edge Function `match-daily` (and batch job that fills `user_daily_match_pool`).

**Flow:**

1. **Dealbreakers** – Age range, gender preferences, onboarding completed. Candidates that fail are filtered out.
2. **Composite score** – Each candidate is scored from:
...
3. **60/40 split** – The daily batch is built as ~60% **similar** (high personality similarity) and ~40% **complementary** (balanced opposites). Tie-breakers: interest overlap, then archetype.
4. **Repeat avoidance** – Prefer not to show the same person two days in a row if there are enough others.
5. **Delivery** – Free users get up to 5 matches per day; Plus/Premium get the full batch. Matches are stored in `matches` and delivered via the `match-daily` Edge Function.

**Relevant code:** `generateUserMatchPool`, `calculateCompositeScore`, `calculateSimilarityScore`, `calculateComplementaryScore` in `src/lib/matching.ts`; `supabase/functions/match-daily/index.ts`.
