...
Before diving into how to construct your app's navigation tree with Expo Router, let's first understand the core concepts. For special filenames and notation (parentheses, brackets, `_layout`, `+not-found`), see [expo-router-notation.md](expo-router-notation.md). that make up the foundation of file-based routing in Expo Router, and how an Expo Router project might differ in structure from other React Native projects.
...
- **src/components/app-tabs.native.tsx** and **src/components/app-tabs.tsx** are platform-specific tab components. The **.native.tsx** file is used on Android and iOS, while the **.tsx** file is used on web. The root layout imports these to render the tab navigator.
...
## Running with Expo Router (this project)

By default this app still uses **Vite** and **React Router** (`main.tsx` → `App.tsx` → `@/routes`). The **src/app** folder is only used when you run with Expo Router and Metro.

To run with Expo Router:

1. **Set entry to Expo Router**  
   In **package.json**, set `"main": "expo-router/entry"` (or use **app.json** / Expo config so the bundler uses `expo-router/entry` as the app entry).

2. **Run with Metro**  
   Start the dev server with Expo (Metro), not Vite:
   ```bash
   npx expo start
   ```
   Or use the script: `npm run expo:start`.

Then the app will use **src/app/_layout.tsx** and file-based routes under **src/app/** instead of React Router. To go back to the normal web/iOS build, restore `"main": "index.js"` (or remove the Expo Router entry) and use `npm run dev` / `npm run build` again.

### Testing Expo Router

For integration tests of Expo Router (mock routes, deep-links, pathname/params assertions), use **expo-router/testing-library** with Jest and **@testing-library/react-native**. See [expo-router-testing.md](expo-router-testing.md).
