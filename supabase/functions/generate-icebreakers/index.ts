...
serve(async (req) => {
  ...
  try {
    ...
    // Likhet/motsats explanation for users (why they are similar or complementary)
    ...
    const matchTypeExplanation =
      matchType === 'similar'
        ? `Ni är en likhetsmatch – ni tillhör samma personlighetskategori (${userCategoryTitle}) och delar liknande värderingar och sätt att se på relationer.`
        : `Ni är en motsatsmatch – era personligheter kompletterar varandra (du ${userCategoryTitle}, ${matchedDisplayName} ${matchedCategoryTitle}), vilket kan ge balans och ny dimension i förhållandet.`;
