...
serve(async (req: Request) => {
  ...
  if (new URL(req.url).pathname.endsWith('/health')) {
    return new Response(JSON.stringify({ status: 'ok', service: 'match-daily', version: 'v1' }), {
  ...
  if (userId) {
    ...
    if (entry && entry.reset > now) {
      if (entry.count >= 5) {
        return new Response(JSON.stringify({ error: 'Rate limit exceeded. Max 5 requests per hour.' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 429
        });
      }
  ...
  try {
    ...
    const PAGE_SIZE = typeof page_size === 'number' && page_size > 0 && page_size <= 50 ? page_size : 5;
    ...
    if (newMatchesToDeliver.length === 0) {
      ...
      if (cursor) {
        const cursorIndex = paginatedRows.findIndex((m) => m.matched_user_id === cursor);
        if (cursorIndex >= 0) {
      ...
      const next_cursor = formattedMatches.length > 0 ? formattedMatches[formattedMatches.length - 1].profile_id : null;
