...
// ...existing code copied from match-daily/index.ts...

// (Full implementation pasted below)
...
serve(async (req: Request) => {
  ...
  if (new URL(req.url).pathname.endsWith("/health")) {
    return new Response(
      JSON.stringify({ status: "ok", service: "match-daily", version: "v1" }),
      {
  ...
  // Parse user_id early for rate limiting
  ...
  if (userId) {
    ...
    if (entry && entry.reset > now) {
      ...
      rateLimitMap.set(key, { count: 1, reset: now + 60 * 60 * 1000 });
    }
  }
  ...
  try {
    ...
    // Accept cursor and page_size for pagination
    const { user_id, cursor, page_size } = await req.json();
    const requestUserId = user_id || user.id;
    const PAGE_SIZE =
      typeof page_size === "number" && page_size > 0 && page_size <= 50
        ? page_size
        : 5;
    ...
    const { data: profile, error: profileError } = await supabaseClient
      .from("profiles")
      .select(
        "onboarding_completed, onboarding_completed_at, subscription_tier"
      )
      .eq("id", requestUserId)
      .single();
    ...
    if (profile.onboarding_completed_at) {
      ...
      const hoursSinceOnboarding =
        (now.getTime() - onboardingTime.getTime()) / (1000 * 60 * 60);

      if (hoursSinceOnboarding < 24) {
        const hoursRemaining = Math.ceil(24 - hoursSinceOnboarding);
        const minutesRemaining = Math.round(
          (24 - hoursSinceOnboarding - Math.floor(24 - hoursSinceOnboarding)) *
            60
        );
        ...
        return new Response(
          JSON.stringify({
            journey_phase: "WAITING",
            message:
              "Din första matchning kommer snart! Vi förbereder dina personliga matchningar.",
            time_remaining: `${hoursRemaining}h ${minutesRemaining}m`,
            next_match_available: new Date(
              onboardingTime.getTime() + 24 * 60 * 60 * 1000
            ).toISOString(),
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!matchPool) {
      return new Response(
        JSON.stringify({
          ...
          matches: [],
          message:
            "Match pool not yet generated for today. Please check back later.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    // 4. Cursor-based pagination for matches
    let paginatedCandidates = candidates;
    if (cursor) {
      // Assume cursor is the last delivered match's userId
      const cursorIndex = candidates.findIndex((c) => c.user.userId === cursor);
      if (cursorIndex >= 0) {
        paginatedCandidates = candidates.slice(cursorIndex + 1);
      }
    }
    const matchesToDeliver = paginatedCandidates.slice(0, PAGE_SIZE);
    ...
    const existingMatchRows: MatchIdRow[] = (existingMatches ??
      []) as MatchIdRow[];
    const alreadyDeliveredIds = new Set(
      existingMatchRows.map((m) => m.matched_user_id)
    );
    ...
    // 6. If already delivered, return existing (with pagination)
    if (newMatchesToDeliver.length === 0) {
      // Fetch existing matches and format as MatchOutput
      const { data: existingMatchData, error: existingError } =
      ...
      // Apply cursor-based pagination to existing matches
      let paginatedRows = existingMatchData ?? [];
      if (cursor) {
        const cursorIndex = paginatedRows.findIndex(
          (m) => m.matched_user_id === cursor
        );
        if (cursorIndex >= 0) {
          paginatedRows = paginatedRows.slice(cursorIndex + 1);
        }
      }
      paginatedRows = paginatedRows.slice(0, PAGE_SIZE);

      const formattedMatches = paginatedRows.map((m, index) => ({
        match_id: m.id,
        ...
        ai_icebreakers: (
          m.icebreakers || ["Hej!", "Hur mår du?", "Vad gör du?"]
        ).slice(0, 3),
        personality_insight:
          m.personality_insight || "Ni delar liknande värderingar",
        match_reason:
          m.match_type === "similar"
            ? "60% liknande värderingar"
            : "40% kompletterande energi",
      ...
      // Set next_cursor to the last delivered match's userId, or null if no more
      const next_cursor =
        formattedMatches.length > 0
          ? formattedMatches[formattedMatches.length - 1].profile_id
          : null;
    ...
    const matchInserts = newMatchesToDeliver.map((match) => ({
      ...
      icebreakers: match.icebreakers?.slice(0, 3) || [
        "Hej!",
        "Hur mår du?",
        "Vad gör du?",
      ], // Always exactly 3
    ...
    if (insertedMatches && insertedMatches.length > 0) {
      const insertedMatchRows = isInsertedMatchRowArray(insertedMatches)
        ? insertedMatches
        : [];
    ...
    const insertedMatchRows: InsertedMatchRow[] = (insertedMatches ??
      []) as InsertedMatchRow[];
    const matchIds = insertedMatchRows.map((m) => m.id);
    await supabaseClient.from("last_daily_matches").upsert({
      user_id: requestUserId,
      date: today,
      match_ids: matchIds,
    });
    ...
    const userMatchRows: MatchRecordId[] = (allUserMatches ??
      []) as MatchRecordId[];
    ...
    const formattedMatches = insertedMatchRows.map((m, index) => ({
      ...
      expires_at: null,
      special_effects:
        index === 0 && isFirstMatchEver ? ["confetti", "celebration"] : null,
      photo_urls: m.photo_urls || [],
      bio_preview: m.bio_preview || "",
      common_interests: m.common_interests || [],
    }));
